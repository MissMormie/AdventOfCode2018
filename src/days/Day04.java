...
public class Day04 {
    ...
    public static void runA() {
        ...
        Map<String, Map<Integer, Integer>> guardMap = new HashMap<>();

        String guard = "";
        int startSleep = 0;
        boolean guardAsleep = false;
        for(String line : inputArray) {
            // New guard
            if(line.contains("Guard")) {
                // old guard didn't wake up
                if(guardAsleep == true) {
                    addSleep(startSleep, 60, guardMap.get(guard));
                    guardAsleep = false;
                }
                guard = line.substring(line.indexOf("#")+1, line.indexOf(" begins shift"));
                if(!guardMap.containsKey(guard)) {
                    guardMap.put(guard, new HashMap<>());
                }
            }

            // Falls asleep
            else if(line.contains("falls asleep")) {
                startSleep = Integer.valueOf(line.substring(15, 17));
                guardAsleep = true;
            }

            // wakes up
            else if(line.contains("wakes up")) {
                int stopSleep = Integer.valueOf(line.substring(15, 17));
                guardAsleep = false;
                addSleep(startSleep, stopSleep, guardMap.get(guard));
            }
        }

        int mostMinutesAsleep = 0;
        String mostSleepyGuard = "";
        int mostSleptMinute = 0;
        // Guard that sleeps the most:
        for(Map.Entry<String, Map<Integer, Integer>> entry: guardMap.entrySet()) {
            int totalSleepMinutes = 0;
            // count minutes asleep.
            int minuteMostAsleep = 0;
            int minuteTimesAsleep = 0;
            for(Map.Entry<Integer, Integer> minuteEntry : entry.getValue().entrySet()) {
                if(minuteTimesAsleep < minuteEntry.getValue()) {
                    minuteMostAsleep = minuteEntry.getKey();
                    minuteTimesAsleep = minuteEntry.getValue();
                }
                totalSleepMinutes += minuteEntry.getValue();
            }
            if(totalSleepMinutes > mostMinutesAsleep) { // new sleepiest guard.
                mostSleepyGuard = entry.getKey();
                mostMinutesAsleep = totalSleepMinutes;
                mostSleptMinute = minuteMostAsleep;
            }
        }

        System.out.println("answer A: guard: " + mostSleepyGuard + " x minutes: " + mostSleptMinute + "  = " + (Integer.valueOf(mostSleepyGuard)) * mostSleptMinute);
        runB(guardMap);
    }

    public static void addSleep(int startSleep, int stopSleep, Map<Integer, Integer> sleepTimer) {
        for(int minute = startSleep; minute < stopSleep; minute++) {
            int count = sleepTimer.getOrDefault(minute, 0)+1;
            sleepTimer.put(minute, count);
        }
    }


    public static void runB(Map<String, Map<Integer, Integer>> guardMap) {
        int guard = 0;
        int minute = 0;
        int timesAsleep = 0;
        for(Map.Entry<String, Map<Integer, Integer>> entry: guardMap.entrySet()) { // per guard
            for(Map.Entry<Integer, Integer> minuteEntry : entry.getValue().entrySet()) { //per minute
                if(minuteEntry.getValue() > timesAsleep) {
                    timesAsleep = minuteEntry.getValue();
                    minute = minuteEntry.getKey();
                    guard = Integer.valueOf(entry.getKey());
                }
            }
        }

        // What is the ID of the guard you chose multiplied by the minute you chose?
        System.out.println("answer B: guard: " + guard + " minute: " + minute + " product: " + guard * minute);
    }

    public static String getMarcoInput() {
        return "[1518-07-10 23:54] Guard #3167 begins shift\n" +
    ...
    public static String getInput() {
        return  "[1518-10-31 00:58] wakes up\n" +
